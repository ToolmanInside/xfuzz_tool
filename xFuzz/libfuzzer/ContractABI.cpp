...
namespace fuzzer {
  ...
  Accounts ContractABI::decodeAccounts() {
    ...
    for (auto account : accounts) {
      ...
      if(balance == 0)
      {
        balance = 0xffffffffff/((rand() % (10000000-1))+ 1);
  ...
  void ContractABI::updateTestData(bytes data) {
    ...
    for (auto &fd : this->fds) {
      ...
      function2Positions[fd.name] = offset;
    }
  }
      ...
      for (auto td : fd.tds) {
        switch(td.dimensions.size()) {
          case 0: {
            ...
            //let address equal 0xf0 to increase the converage
  ...
  vector<bytes> ContractABI::encodeFunctions() {
    ...
    for (auto fd : fds) {
        ...
        //if(selector.empty()) selector=bytes(0,0);
        //cout<<"      "<<selector<<endl;
  ...
  bytes ContractABI::functionSelector(string name, vector<TypeDef> tds) {
    ...
    //cout<<"  function and selector:  "<<bytes(fullSelector.begin(), fullSelector.begin() + 4)<<" "<<signature<<endl;
  ...
  void ContractABI::setreentrancyfunction(string functionname)
  {
    ...
    {
      if(fds[i].name==functionname)
      {
        //cout<<fds[i].name<<endl;
        tempfd=fds[i];
        tempfd.hasreentrancy=true;
        auto iter=fds.erase(fds.begin()+i);
        flag=true;
        //ContractABI::TargetFunctionSelector=functionSelector(fds[i].name,fds[i].tds);
        
      }
      if(fds[i].name=="") constructorindex=i;
    }
    if(flag) fds.push_back(tempfd);
    
  }

  bytes ContractABI::getTargetfunction()
  {
    //return TargetFunctionSelector;
    for(auto iter : fds)
    {
      if(iter.hasreentrancy)
      {
        bytes selector=functionSelector(iter.name,iter.tds);
        iter.name;
        return selector;
      }
